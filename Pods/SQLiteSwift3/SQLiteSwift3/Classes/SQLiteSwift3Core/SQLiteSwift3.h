...
/**
 run sql return data array.
 
 @param sql run sql
 @param fieldsArray select field
 @finish block of handle the result
 
 */
-(void)runSelectSQL:(nonnull NSString *)sql selectField:(nonnull NSArray<NSString *> *)fieldsArray searchFinish:(nullable void (^)(BOOL success, NSArray<NSDictionary<NSString *,id> *> * _Nullable resultArray))finish;

/**
 run sql no query.
 
 @param sql selectFieldsArray
 @return result of the sql run operation
 
 */
-(BOOL)runSQL:(nonnull NSString *)sql;
