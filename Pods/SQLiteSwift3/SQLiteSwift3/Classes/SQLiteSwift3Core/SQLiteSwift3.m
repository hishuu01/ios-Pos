...
-(void)runSelectSQL:(NSString *)sql selectField:(NSArray<NSString *> *)fieldsArray searchFinish:(nullable void (^)(BOOL, NSArray<NSDictionary<NSString *,id> *> * _Nullable))finish{
    if([self serviceRuning]){
        //begin query sql
        NSMutableArray * resultArray = [NSMutableArray array];
        if ([self runSQLforSelect:sql selectField:fieldsArray writeTo:resultArray]) {
            if (finish) {
                finish(YES,resultArray);
            }
        }else{
...
/**
 ...
 @return wether success
 */
-(BOOL)runSQLforSelect:(NSString *)sql selectField:(NSArray *)selectFieldsArray writeTo:(NSMutableArray *)resultArray{
    sqlite3_stmt * stmt = nil;
    int code = sqlite3_prepare_v2(self->sqlite, [sql UTF8String], -1, &stmt, NULL);
    if (code!=SQLITE_OK) {
        return NO;
        ...
        NSMutableArray * fieldsArray = [NSMutableArray arrayWithArray:selectFieldsArray];
        if (selectFieldsArray==nil) {
            NSString * info = [NSString stringWithFormat:@"parameter selectFieldsArray error in:%@",fieldsArray];
            LOG_FORMAT(NSStringFromClass(self.class), info);
            return NO;
        }
        while (sqlite3_step(stmt)==SQLITE_ROW) {
